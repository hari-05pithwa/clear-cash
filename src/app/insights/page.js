...
import { useRouter } from "next/navigation";
import { PieChart as RePie, Pie, Cell, ResponsiveContainer } from "recharts";
import { Target, TrendingDown, Home, PieChart, History, Settings, BarChart3, ChevronDown, Zap, Filter } from "lucide-react";
import { formatCurrency } from "@/lib/utils";
import { motion, AnimatePresence } from "framer-motion";
...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }) => {
  ...
  const y = cy + radius * Math.sin(-midAngle * (Math.PI / 180));
  return (
    <text x={x} y={y} fill="white" textAnchor="middle" dominantBaseline="central" className="text-[11px] font-black tracking-tighter">
      {percent > 0.05 ? `${(percent * 100).toFixed(0)}%` : ""}
    </text>
  );
};
...
export default function Insights() {
  ...
  const filteredStats = useMemo(() => {
    if (activeFilter === "All") return rawStats;
    return rawStats.filter(item => item._id?.toLowerCase() === activeFilter.toLowerCase());
  }, [rawStats, activeFilter]);
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] p-6 pb-44 selection:bg-blue-100 overflow-x-hidden relative">
      ...
      <header className="pt-10 mb-8 relative z-[100]">
        <div className="flex justify-between items-start mb-6">
          <div>
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] mb-1">Vault Analytics</p>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none">Spending Split</h1>
          </div>
          <motion.button 
            whileTap={{ scale: 0.9 }}
            onClick={() => setIsFilterOpen(!isFilterOpen)}
        ...
        <section className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {DATE_FILTERS.map((f) => (
            <button
              ...
              onClick={() => setDateFilter(f)}
              className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                dateFilter === f ? "bg-blue-600 text-white shadow-md shadow-blue-100 border-blue-600" : "bg-white text-slate-400 border-slate-100"
              }`}
            >
              {f}
            </button>
        ...
        <AnimatePresence>
          {isFilterOpen && (
            ...
            >
              {FILTER_CATEGORIES.map((cat) => (
                <button 
                  key={cat} onClick={() => { setActiveFilter(cat); setIsFilterOpen(false); }}
                  className={`w-full text-left px-5 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-colors ${activeFilter === cat ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:bg-slate-50"}`}
                >
                  {cat}
                </button>
      ...
      <AnimatePresence mode="wait">
        {activeFilter === "All" && rawStats.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-24 bg-white rounded-[4rem] shadow-2xl border border-slate-50 flex flex-col items-center">
            <BarChart3 size={48} className="text-blue-600 mb-6" />
            <h2 className="font-black text-2xl text-slate-900 uppercase tracking-tighter">Vault Empty</h2>
            <p className="text-slate-400 text-sm mt-2 mb-10 px-10 text-center leading-relaxed font-medium">No chronological records found for <span className="text-blue-600 font-bold">{dateFilter}</span>.</p>
            <button onClick={() => router.push("/dashboard")} className="bg-slate-900 text-white px-10 py-5 rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-xl">Go to Vault</button>
          </motion.div>
        ) : activeFilter === "All" ? (
          <motion.div key="all" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            <section className="bg-white rounded-[3.5rem] p-8 border border-slate-50 shadow-xl relative overflow-hidden">
              <div className="absolute top-8 right-8 z-20 bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex flex-col gap-2 shadow-sm">
                {rawStats.map((item, index) => (
                  <div key={item._id} className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                    <span className="text-[9px] font-black text-slate-600 uppercase truncate max-w-[65px]">{item._id}</span>
                  </div>
                ))}
              </div>
              <div className="h-[340px] w-full relative">
                <ResponsiveContainer width="100%" height="100%">
                  <RePie>
                    <Pie data={rawStats} innerRadius={90} outerRadius={130} paddingAngle={4} dataKey="total" nameKey="_id" stroke="none" label={renderCustomizedLabel} labelLine={false} isAnimationActive={false}>
                      {rawStats.map((entry, index) => (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
                  <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">{dateFilter}</p>
                  <p className="text-xl font-black text-slate-900 tracking-tighter uppercase leading-none">Portfolio</p>
                </div>
              </div>
            </section>
            <div className="space-y-3">
              {rawStats.map((item, index) => (
                <div key={item._id} className="bg-white p-5 rounded-[2rem] border border-slate-50 flex justify-between items-center shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className="w-3.5 h-3.5 rounded-full border-2 border-white shadow-md" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                    <p className="font-black text-slate-900 text-[13px] uppercase tracking-tight">{item._id}</p>
                  </div>
          ...
          <motion.div key="cat" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="pt-4 flex flex-col items-center justify-center min-h-[50vh]">
            <div className="w-full bg-slate-900 rounded-[4rem] p-12 text-white shadow-2xl relative overflow-hidden border border-white/5">
              <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-[80px] -mr-32 -mt-32" />
              <div className="relative z-10 flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-white/5 rounded-3xl flex items-center justify-center mb-8 border border-white/10">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400/60 mb-3">Total Spent</p>
                <h2 className="text-6xl font-black tracking-tighter mb-8 italic uppercase leading-none">{activeFilter}</h2>
                <div className="h-[1px] w-24 bg-gradient-to-r from-transparent via-blue-500 to-transparent mb-8 opacity-30" />
                <p className="text-5xl font-black tracking-tighter mb-2">{formatCurrency(currentCategorySpend)}</p>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30 italic">{dateFilter} Vault</p>
              </div>
            </div>
            <button onClick={() => setActiveFilter("All")} className="mt-12 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-slate-200 pb-1">Portfolio Overview</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-2xl border-t border-slate-50 px-10 py-5 pb-10 z-[90] flex justify-between items-center shadow-2xl">
        ...
        <button className="flex flex-col items-center gap-1.5 text-blue-600 outline-none"><PieChart size={26} strokeWidth={2.5} /><span className="text-[9px] font-black uppercase tracking-tighter">Insights</span></button>
        ...
        <button onClick={() => router.push("/settings")} className="flex flex-col items-center gap-1.5 text-slate-400 outline-none"><Settings size={26} strokeWidth={2.5} /><span className="text-[9px] font-black uppercase tracking-tighter">Vault ID</span></button>
      </nav>
    </div>
  );
}