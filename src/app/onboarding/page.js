...
export default function Onboarding() {
  ...
  // Helper to restrict input to a-z and A-Z only
  ...
  const handleFinalize = async (e) => {
    ...
    try {
      // 1. Initialize the Vault (Transactions & Balance)
      ...
      if (setupRes.ok) {
        // 2. SAVE PROFILE TO CLOUD (Step 2 Implementation)
        ...
        localStorage.setItem("cc_first_name", firstName.trim());
        localStorage.setItem("cc_last_name", lastName.trim());
        localStorage.setItem("cc_setup_complete", "true");
        
        setIsSuccess(true);
        toast.success("Vault Signature Secured");
        ...
        <AnimatePresence mode="wait">
          {step === 1 ? (
            ...
            >
              ...
              <h1 className="text-4xl font-black tracking-tighter mb-3 text-slate-900 leading-tight">Identity.</h1>
              <p className="text-slate-400 font-medium text-sm mb-12 px-8 leading-relaxed">
                Personalize your private vault. Every entry is end-to-end encrypted.
              </p>

              <div className="space-y-4 mb-12">
                <div className="group relative bg-white border border-slate-100 rounded-[2.2rem] p-1.5 shadow-sm focus-within:shadow-xl transition-all">
              ...
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-slate-100 pb-1"
                >
            ...
            <motion.div
              ...
              className="w-full max-w-sm text-center"
            >
              <div className="mb-10 inline-flex p-6 bg-white rounded-[2.5rem] shadow-xl text-blue-600">
                <Wallet size={40} strokeWidth={1.5} />
              </div>

              <h1 className="text-4xl font-black tracking-tighter mb-3">Vault Value.</h1>
              <p className="text-slate-400 font-medium text-md mb-12">What is your current bank balance?</p>
              ...
              <div className="relative mb-16 px-4">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <input
                    ...
                    autoFocus
                    placeholder="0"
                    style={{ fontSize: balance.length > 8 ? "2.5rem" : "4.2rem" }}
              ...
              <motion.button
                ...
                className="w-full h-20 bg-blue-600 text-white rounded-[2.5rem] font-black uppercase tracking-[0.15em] text-[13px] shadow-2xl flex items-center justify-center gap-3 disabled:opacity-20 transition-all"
              >
                {loading ? <Loader2 className="animate-spin" /> : "Authorize Vault"}
              </motion.button>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          >
            {!isSuccess ? (
              <div className="flex flex-col items-center">
                <Loader2 size={64} className="text-blue-600 animate-spin mb-8" strokeWidth={1.5} />
                <h2 className="text-xl font-black uppercase tracking-tighter text-slate-900">Securing Signature</h2>
                <p className="text-sm font-medium text-slate-400 mt-2">Generating unique Vault ID...</p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} className="w-24 h-24 bg-emerald-500 rounded-[2.8rem] flex items-center justify-center shadow-2xl mb-8">
                  <CheckCircle2 size={48} className="text-white" strokeWidth={2.5} />
                </motion.div>
                <h2 className="text-3xl font-black uppercase tracking-tighter text-slate-900">Success</h2>
                <p className="text-sm font-bold text-slate-400 mt-2">Vault Protocol Initialized</p>
              </div>
            )}
          </motion.div>
        )}
